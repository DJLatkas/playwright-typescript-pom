...
test('Deal', async({request})=>{
  ...
  function hasBlackjack(cards: any[]) {
    const values = cards.map((card: { value: any; }) => card.value);

    const acesCount = values.filter((value: string) => value === 'ACE').length;
    const nonAceTotal = values
      .filter((value: string) => value !== 'ACE')
      .reduce((total: number, value: string) => total + (['KING', 'QUEEN', 'JACK'].includes(value) ? 10 : parseInt(value, 10)), 0);
